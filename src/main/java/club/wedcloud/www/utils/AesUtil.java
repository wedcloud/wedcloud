...
public class AesUtil {
   ...
   * @param password 加密密码
   * @param keySize
   *        密钥长度16,24,32(密码长度为24和32时需要将local_policy.jar/US_export_policy.jar两个jar包放到JRE目录%jre%/lib/security下)
   * @return
   */
  public static byte[] encrypt(String content, String password, int keySize) {
    try {
      ...
      SecretKeySpec key = new SecretKeySpec(
          ZeroPadding(password.getBytes(Base64Util.DEFAULT_CHARSET), keySize), "AES");
      ...
      IvParameterSpec iv =
          new IvParameterSpec(ZeroPadding(INITIAL_VECTOR.getBytes(Base64Util.DEFAULT_CHARSET), 16));
